...
class Utility {
    ...
    getVisibility (systemUsed, visibilityInKm) {
        return systemUsed == "metric" ? (visibilityInKm / 1000).toPrecision(2): Converter.kmToM(visibilityInKm / 1000);
    }
    
    getTime (systemUsed, currentTime, timezone) {
        return systemUsed == "metric" ? `${parseInt(Converter.convertTime(currentTime, timezone)[0].split(":")[0])}:${Converter.convertTime(currentTime, timezone)[0].split(":")[1]}`: Converter.timeToAMPM(convertTime(currentTime, timezone)[0]);
    }
